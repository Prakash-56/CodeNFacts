    ...
    >
      <div className="snip-header">
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
          ...
          {s.likes > 1400 && (
            <span style={{ fontSize: "0.65rem", background: "#f472b620", color: "#f472b6", padding: "1px 6px", borderRadius: "999px" }}>
              ★ PRO
            </span>
          )}
        </div>
      ...
      <div
        className="snip-card-glow"
        style={{ background: `radial-gradient(circle at 50% 20%, ${langColor}22, transparent 70%)` }}
      />
    </div>
  );
}
...
function SearchBar({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="explore-search-wrap">
      ...
      />
      {value && (
        <button className="explore-search-clear" onClick={() => onChange("")}>✕</button>
      )}
    </div>
  );
}
...
/* ── Difficulty Filters ── */
// FIX: onChange typed as (v: Difficulty) => void to match setDiffFilter's expected type
function DiffFilters({ value, onChange }: { value: Difficulty; onChange: (v: Difficulty) => void }) {
  const levels: Difficulty[] = ["all", "Beginner", "Intermediate", "Advanced"];
  return (
...
export default function ExplorePage() {
  ...
  const relatedSnippets = selected
    ? SNIPPETS.filter(s => s.id !== selected.id && s.lang === selected.lang).slice(0, 3)
    : [];
    ...
    <>
      <style>{`
        ...
        .matrix-canvas {
          ...
          pointer-events: none; opacity: 0.13; mix-blend-mode: screen;
        }
        .explore-mesh {
        ...
        .explore-content {
          ...
          max-width: 1380px; margin: 0 auto; padding: 0 1.75rem;
        }
        .explore-banner { padding: 6.5rem 0 4rem; text-align: center; }
        .explore-eyebrow {
        ...
        .explore-title {
          ...
          line-height: 1.05; letter-spacing: -0.04em; margin-bottom: 1.4rem;
        }
        ...
        .explore-search {
          ...
          border: 1px solid rgba(148,163,184,0.15); border-radius: 9999px;
          padding: 1.05rem 3.2rem 1.05rem 3.8rem;
          font-size: 1rem; color: #e2e8f0; backdrop-filter: blur(12px);
          outline: none;
        }
        ...
        .explore-cat-btn, .explore-diff-btn {
          ...
          background: rgba(15,23,42,0.4); color: rgba(226,232,240,0.7);
          font-size: 0.86rem; cursor: pointer;
          transition: all 0.25s cubic-bezier(0.23,1,0.32,1); white-space: nowrap;
        }
        .explore-cat-btn:hover, .explore-diff-btn:hover {
          background: rgba(99,102,241,0.1);
          border-color: rgba(129,140,248,0.4); color: #e0e7ff;
        }
        .explore-cat-btn[data-active="true"], .explore-diff-btn[data-active="true"] {
          background: linear-gradient(135deg, #6366f1, #8b5cf6);
          border-color: #a5b4fc; color: #fff;
        ...
        .snip-card {
          position: relative; background: rgba(15,23,42,0.72);
          border: 1px solid rgba(148,163,184,0.1); border-radius: 20px;
          padding: 1.65rem; backdrop-filter: blur(22px);
        ...
        .snip-lang-badge {
          ...
          padding: 0.25rem 0.85rem; border-radius: 999px; border: 1px solid;
          font-family: 'JetBrains Mono', monospace;
        }
        .snip-diff-badge { font-size: 0.73rem; font-weight: 600; letter-spacing: 0.5px; }
        .snip-title {
          font-family: 'Syne', sans-serif; font-size: 1.22rem;
          font-weight: 700; line-height: 1.35; margin-bottom: 0.6rem;
        }
        .snip-desc { font-size: 0.875rem; color: #94a3b8; line-height: 1.65; margin-bottom: 1.25rem; }
        .snip-code-wrap {
          background: #0a0f1f; border: 1px solid #1e2937;
          border-radius: 12px; overflow: hidden; margin-bottom: 1.1rem;
        }
        .snip-code-topbar {
          background: #111827; padding: 0.55rem 1rem;
          display: flex; align-items: center; justify-content: space-between; font-size: 0.7rem;
        }
        .snip-pre {
          margin: 0; padding: 1.1rem 1.25rem;
          font-family: 'JetBrains Mono', monospace;
          font-size: 0.8rem; line-height: 1.75; color: #e2e8f0;
          overflow-x: auto; white-space: pre;
        }
        .code-line { display: flex; gap: 2.2rem; }
        .line-number { width: 2.2rem; text-align: right; color: #475569; user-select: none; flex-shrink: 0; }
        .line-content { flex: 1; }
        .snip-tags { display: flex; flex-wrap: wrap; gap: 0.35rem; margin-bottom: 1.1rem; }
        .snip-tag {
          font-size: 0.7rem; background: rgba(99,102,241,0.1); color: #a5b4fc;
          padding: 0.15rem 0.65rem; border-radius: 4px; font-family: 'JetBrains Mono', monospace;
        }
        .snip-footer {
          display: flex; justify-content: space-between; align-items: center;
          padding-top: 0.9rem; border-top: 1px solid rgba(148,163,184,0.1);
        }
        ...
        .modal-content {
          ...
          width: 100%; max-width: 1020px; max-height: 94vh; overflow: hidden;
          box-shadow: 0 30px 120px -20px rgb(99 102 241 / 40%);
        }
        .modal-header {
          padding: 1.5rem 2rem; border-bottom: 1px solid #1e2937;
          display: flex; align-items: center; justify-content: space-between;
        }
        .modal-close {
          width: 38px; height: 38px; border-radius: 9999px;
          background: rgba(148,163,184,0.1); display: grid; place-items: center;
          font-size: 1.4rem; cursor: pointer; transition: all 0.2s; border: none; color: white;
        }
        .modal-close:hover { background: #ef4444; }
        .modal-body {
          padding: 2rem; overflow-y: auto; max-height: calc(94vh - 120px);
        }
        .modal-related { margin-top: 3rem; border-top: 1px solid #1e2937; padding-top: 2rem; }
        .modal-related-grid {
          display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1rem;
        }
        .explore-empty { text-align: center; padding: 6rem 2rem; color: #64748b; }
      ...
      <div className="explore-root">
        ...
        <div className="explore-content">
          ...
          {/* FIX: setDiffFilter is now compatible because DiffFilters onChange is (v: Difficulty) => void */}
        ...
        <div className="explore-modal" onClick={() => setSelected(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <span
                  className="snip-lang-badge"
                  style={{
                    background: `${LANG_COLOR[selected.langLabel] || "#6366f1"}22`,
                    color: LANG_COLOR[selected.langLabel] || "#6366f1",
                  }}
                >
                  {selected.langLabel}
                </span>
            ...
            <div className="modal-body">
              ...
              <div
                style={{
                  ...
                  padding: "1.5rem 0", borderTop: "1px solid #1e2937", borderBottom: "1px solid #1e2937",
                }}
              >
                <div>
              ...
              <div
                style={{
                  ...
                  background: "rgba(99,102,241,0.08)", borderRadius: "16px",
                  border: "1px solid rgba(99,102,241,0.2)",
                }}
              >
                <div style={{ fontSize: "0.75rem", color: "#818cf8", letterSpacing: "1px", marginBottom: "0.6rem" }}>
                  ✦ CodeNFacts INSIGHT
                </div>
              ...
              {relatedSnippets.length > 0 && (
                <div className="modal-related">
                  <div style={{ fontSize: "1.05rem", marginBottom: "1rem", color: "#e2e8f0" }}>
                    Related in {selected.langLabel}
                  </div>
            ...
            <div
              style={{
                padding: "1.5rem 2rem", borderTop: "1px solid #1e2937",
            ...
            >
              ...
              <button
                ...
                style={{
                  flex: 1, padding: "0.95rem", borderRadius: "12px",
